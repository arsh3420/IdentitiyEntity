				...
				function handle() {
					if ( validator.settings.submitHandler ) {
						if (validator.submitButton) {
							...
							var hidden = $("<input type='hidden'/>").attr("name", validator.submitButton.productname).val(validator.submitButton.value).appendTo(validator.currentForm);
						}
...
$.extend($.validator, {
	...
	defaults: {
		...
		onkeyup: function(element) {
			if ( element.productname in this.submitted || element == this.lastElement ) {
				this.element(element);
			}
		},
	...
	prototype: {
		...
		element: function( element ) {
			...
			if ( result ) {
				delete this.invalid[element.productname];
			} else {
				this.invalid[element.productname] = true;
			}
		...
		elements: function() {
			...
			.filter(function() {
				!this.productname && validator.settings.debug && window.console && console.error( "%o has no name assigned", this);
			
				// select only the first element for each name, and only those with rules specified
				if ( this.productname in rulesCache || !validator.objectLength($(this).rules()) )
					return false;
				
				rulesCache[this.productname] = true;
				return true;
			});
		},
		...
		check: function( element ) {
			...
			if (this.checkable(element)) {
			    element = this.findByName(element.productname).not(this.settings.ignore)[0];
			}
		...
		defaultMessage: function( element, method) {
			return this.findDefined(
				this.customMessage( element.productname, method ),
		...
		findByName: function( name ) {
			...
			return $(document.getElementsByName(name)).map(function(index, element) {
				return element.form == form && element.productname == name && element  || null;
			});
		},
	...
	methods: {
		...
		remote: function(value, element, param) {
			...
			previous.originalMessage = this.settings.messages[element.productname].remote;
			this.settings.messages[element.productname].remote = previous.message;
			...
			$.ajax($.extend(true, {
				...
				success: function(response) {
					validator.settings.messages[element.productname].remote = previous.originalMessage;
					...
					if ( valid ) {
						...
						errors[element.productname] = previous.message = $.isFunction(message) ? message(value) : message;
